...
import { Plus, Workflow as WfIcon, Pencil, Trash2 } from "lucide-react";
import { toast } from "sonner";
import { useServerFn } from "@tanstack/react-start";
import { listWorkflows, createWorkflow, deleteWorkflow, updateWorkflow } from "@/server/workflow.functions";
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                <Label>Nome</Label>
                <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ex: Atendimento Trabalhista" />
              </div>
              <div>
            ...
            <Card key={w.id} className="hover:shadow-md transition-shadow">
              ...
              <CardContent>
                ...
                <div className="flex gap-2">
                  <Link to="/workflows/$id" params={{ id: w.id }} className="flex-1">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => handleDelete(w.id)}><Trash2 className="h-4 w-4" /></Button>
                </div>
              </CardContent>
            </Card>
          ))}
